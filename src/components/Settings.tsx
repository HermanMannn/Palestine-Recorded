...
export default function Settings() {
  ...
  // The username we use to find the account on initial load
  // If you changed your username, update "ahmed" to match your DB!
  const [loggedInUsername, setLoggedInUsername] = useState("ahmed");
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          console.error(`User '${loggedInUsername}' not found in DB.`);
          showToast(`User '${loggedInUsername}' not found in database!`, "error");
        }
      } catch (error) {
  ...
  // --- SAFE PROFILE SAVE HANDLER ---
  const handleProfileUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!userKey) {
      alert(`Error: Cannot save. The app couldn't find the user '${loggedInUsername}' in the Firebase database.`);
      return;
    }
    ...
    try {
      ...
      // Package the data safely
      const updateData: any = {
        username: username || "",
        bio: bio || ""
      };

      // Only attach the profile picture if one actually exists
      if (profilePic) {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!userKey) {
      showToast("Cannot save: No user data loaded.", "error");
      return;
    }
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setProfilePic(reader.result as string); // Saves as Base64 text
        showToast("Picture selected! Click 'Save Changes' to apply.");
      };
